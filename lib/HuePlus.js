                    ...
                    for (let i = 0; i < list.length; i++) {
                        if (list[i].vendorId.toLowerCase() === "04d8" && list[i].productId.toLowerCase() === "00df") {
                            port = new SerialPort(list[i].comName, { baudRate: 256000}, (err) => {
                                ...
                                write([0xc0]).then(async () => {
                                    try {
                                        gLedCount.ch1 = await getLEDCount(1);
                                        gLedCount.ch2 = await getLEDCount(2);
                                        gLedCount.ch0 = gLedCount.ch1 + gLedCount.ch2;
                                        return resolve(t);
                                    } catch (e) {
                                        reject(e);
                                    }
...
function changeColor(color, mode, channel, speed = 3, size = 3, moving = false, backwards = false, custom = false, i = 0, ledCount) {
    return new Promise(async (resolve, reject) => {
        ...
        if (channel === 0) ledCount = gLedCount.ch0; else ledCount = gLedCount["ch" + channel.toString()];
