        ...
        port.open((err) => {
            ...
            port.write(buffer, (err) => {
                ...
                waitUntil(timeout, 100, alt ? () => { return output.length >= 5 } : () => { return output[0] === 1 }, result => {
                    port.close(() => {
                        if (result) {
                            setTimeout(() => { return resolve(alt ? output[output.length - 1] : output) }, timeout);
                        } else {
...
function spectrum(channel, speed = 3) {
    return new Promise((resolve, reject) => {
        changeColor("ffffff", modes.spectrum, channel, speed).then(resolve).catch(reject);
    })
...
function getLEDCount(channel = 1) {
    return new Promise((resolve, reject) => {
        write(Buffer.from([0x8d, channel]), true).then(resolve).catch(reject);;
    })
}
...
function changeColor(color, mode, channel, speed = 3, size = 3, moving = false, backwards = false, custom = false, i = 0, ledCount) {
    return new Promise(async (resolve, reject) => {
        ...
        if (ledCount === undefined) { // This if statement makes the function faster when using multiple colors
            try {
                if (channel === 0) {
                    ledCount = await getLEDCount(1) + await getLEDCount(2);
                } else ledCount = await getLEDCount(channel);
            } catch (e) {
                return reject(new HuePlusError("Could not get the number of LEDs connected to the device: " + e));
            }
        }
